...
parameters {
  vector[4] mu_pr;               // Group means: a, tau, d1_intercept, d1_slope, d2_intercept, d2_slope
  vector<lower=0>[4] sigma;      // Group std devs
...
transformed parameters {  
  ...
  for (i in 1:N) {
    ...
    slope[i]     = mu_pr[4] + sigma[4] * d2_pr[i];
  }
}
...
model {
  ...
  // 개인 파라미터 priors
  // (여기 생략되어 있을 수 있지만 추정하는 경우 들어있어야 함)
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      // 전체 기준 표준화를 사용한 drift rate
      real drift = intercept[i] + slope[i] * (subtlex[i, t] - subtlex_mean) / subtlex_sd;
      if (r == 1)  // upper boundary
        RT[i, t] ~ wiener(a[i], tau[i], 0.5, drift);
      else if (r == 2)  // lower boundary
        RT[i, t] ~ wiener(a[i], tau[i], 0.5, -drift);
    }
  }
}
